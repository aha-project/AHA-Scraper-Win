...
function IsNumeric ($Value) { return $Value -match "^[\d\.]+$" }
...
function GetHandles #calls out to handles to get the handles (synchronously), minimally parses and stores results for future finalization
{
	...
	foreach ($HandleLine in $HandleObjects) #turn each line of the imported data into a hashtable
	{
		...
		if ( $HandleLine.Length -lt 4) { continue; }
...
function Write-Output
{
	...
	{ #Get-Member -MemberType Properties | Select-Object -exp 'Name' |
		$row.keys |  ForEach-Object { $TempCols[$_]=$_ } #copy column names from line 0 of the output data into a new hash table so we can work on formatting
	}
