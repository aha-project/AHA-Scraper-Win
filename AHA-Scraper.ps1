...
$AHAScraperVersion = 'v0.8.5b5'						 #This script tested/requires powershell 2.0+, tested on Server 2008R2, Server 2016.
...
$everything = [Diagnostics.Stopwatch]::StartNew()
...
$BinaryScanResults=@{}
$BinaryScanError=@{ 'ARCH'='ScanError';'ASLR'='ScanError';'DEP'='ScanError';'Authenticode'='ScanError';'StrongNaming'='ScanError';'SafeSEH'='ScanError';'ControlFlowGuard'='ScanError';'HighentropyVA'='ScanError';'DotNET'='ScanError';'SHA512'='ScanError';'SHA256'='ScanError';'SHA1'='ScanError';'MD5'='ScanError' }
...
ForEach ( $exePath in $exepaths ) 
{
	...
	try #the try is out here, because the expectation is that if we fail at any part in here, the failure is with Get-PESecurity
    {
		...
		{ 
			...
			if ($stream) { Write-Host ('Failed at Test-ProcPriv line: {0} Error: {1}' -f @($Error[0].InvocationInfo.ScriptLineNumber, $Error[0])) }  #TODO: error printing is screwed up i think.  #if the stream never existed then we just assume we're trying to scan "system" or "unknown"
		}
    }
	catch 
	{ 
...
$everything.Stop()
Write-Host ('Complete, elapsed time: {0}.' -f @($everything.Elapsed))
...
$outputData | Select-Object ProcessName, PID, ProcessPath, Protocol, LocalAddress, LocalPort, LocalPortName, RemoteAddress, RemotePort, RemoteHostName, RemotePortName, State, ProductName, FileDescription, FileVersion, Company, ProcessCreatedOn, UserName, ProcessServices, ProcessAttributes, DetectionTime, CreationTimestamp, SentBytes, SentPackets, ReceivedBytes, ReceivedPackets, ModuleFilename, ARCH, ASLR, DEP, Authenticode, StrongNaming, SafeSEH, ControlFlowGuard, HighentropyVA, DotNET, PrivilegeLevel, Privileges, SHA512, SHA256, SHA1, MD5, AHAScraperVersion | Export-csv $BinaryAnalysisFile -NoTypeInformation -Encoding UTF8
